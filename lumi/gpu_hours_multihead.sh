#!/bin/bash -l
# Report GPU (GCD) hours for the multi-head RoBERTa fine-tuning job.
#   usage: bash lumi/gpu_hours_multihead.sh [MODEL_DIR]
# MODEL_DIR defaults to the scratch checkpoint dir used by lumi/train_multihead.sh.
# sacct reports zero GCD-hours on LUMI, so we rely on the pure train_runtime written to
# train_metrics.json by nlp_cyber_ner/modeling/train_hf_multihead.py (compute only).
set -euo pipefail

DEFAULT_DIR="/scratch/project_465002928/$(whoami)/retrained/multihead-roberta"
...
print(f"  num_datasets    = {d.get('num_datasets')}")
