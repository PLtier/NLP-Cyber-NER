...
class TokenModel_tiedEMB_sepLSTM(nn.Module):
...
def train_tokenmodel(model, train_loaders, total_batches, sampling_probs, epochs, device, datasets, max_grad_norm) -> TokenModel_tiedEMB_sepLSTM:
...
if __name__ == '__main__' and DEDUPLICATION == False:
    ...
    mlflow.set_experiment("TokenModel_tiedEMB_sepLSTM")
    ...
    with mlflow.start_run(run_name="TokenModel_tiedEMB_sepLSTM_train"):
        # Log hyperparameters
        hyperparams = {
        ...
        model = TokenModel_tiedEMB_sepLSTM(len(common_idx2word), DATASET_LABEL_SIZES)
        ...
        for dataset_name in DATASETS:
            ...
            with mlflow.start_run(run_name=f"TokenModel_tiedEMB_sepLSTM_eval_{dataset_name}", nested=True):
...
if __name__ == '__main__' and DEDUPLICATION == True:
    ...
    for intended_eval_dataset in DATASETS:
        ...
        mlflow.set_experiment(f"train-TokenModel_tiedEMB_sepLSTM-eval-{intended_eval_dataset}")
        with mlflow.start_run(run_name=f"train-TokenModel_tiedEMB_sepLSTM-eval-{intended_eval_dataset}"):
            # Log hyperparameters
            hyperparams = {
                "BATCH_SIZE": BATCH_SIZE,
                "DIM_EMBEDDING": DIM_EMBEDDING,
