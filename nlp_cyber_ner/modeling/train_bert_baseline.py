...
def main():
    try:
        ...
        # Configuration
        config = {
            "base_model_name": "dslim/bert-base-NER",  # Pre-trained NER model
            "max_length": 128,
            "batch_size": 16,
            "learning_rate": 5e-5,
            "epochs": 3,
            "output_dir": "bert_ner_baseline_output",
            "target_train_data": "data/interim/YOUR_FOLDER/YOUR_train_DATA.cleaned",
            "target_dev_data": "data/interim/YOUR_FOLDER/YOUR_dev_DATA.cleaned",
            "target_test_data": "data/interim/YOUR_FOLDER/YOUR_TEST_DATA.cleaned",
            "predictions_output_dir": "artifacts/predictions/bert_baseline/"
        }
        ...
        labels_to_id = {label: i for i, label in enumerate(target_labels)}
        ...
        # Extract BERT backbone from pre-trained NER model (same as LST-NER approach)
        pretrained_ner_model = AutoModelForTokenClassification.from_pretrained(config["base_model_name"])
        bert_backbone = pretrained_ner_model.bert
        
        # Create new model with target labels using base BERT config
        model = AutoModelForTokenClassification.from_pretrained(
            "bert-base-cased",
            num_labels=len(target_labels)
        )
        
        # Replace with NER-experienced BERT backbone
        model.bert = bert_backbone
        model.to(device)
        ...
        if os.path.exists(os.path.join(best_model_path, "pytorch_model.bin")):
            ...
            final_eval_results = evaluate_model(model, dev_dataloader, device, target_id2label)
