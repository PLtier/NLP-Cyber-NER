...
from nlp_cyber_ner.dataset import read_iob2_file, preds_to_tags, transform_dataset, remove_leakage
...
class TokenModel_sepEMB_tiedLSTM(nn.Module):
    def __init__(self, dataset_vocab_sizes, dataset_label_map):
        ...
        # This is the shared bidirectional LSTM - like in our original model
    ...
    def forward(self, input_data, dataset_name):
        ...
        if dataset_name not in self.embeddings:
            raise ValueError(
                f"Dataset id '{dataset_name}' not recognized. "
        ...
        # Copy pasted from original model in project.ipynb
...
if __name__ == "__main__" and DEDUPLICATION == False:
    ...
    (
        ...
        attackner_idx2label,
        attackner_max_len,
    ) = transform_dataset(attackner_train_data, attackner_dev_data, attackner_test_data)
    ...
    (
        ...
        aptner_idx2label,
        aptner_max_len,
    ) = transform_dataset(aptner_train_data, aptner_dev_data, aptner_test_data)
    ...
    (
        ...
        dnrti_idx2label,
        dnrti_max_len,
    ) = transform_dataset(dnrti_train_data, dnrti_dev_data, dnrti_test_data)

    # Dataloaders:
    TRAIN_LOADERS = {
        "dnrti": DataLoader(TensorDataset(dnrti_train_X, dnrti_train_y), BATCH_SIZE, shuffle=True),
        "aptner": DataLoader(
            TensorDataset(aptner_train_X, aptner_train_y), BATCH_SIZE, shuffle=True
        ),
        "attackner": DataLoader(
            TensorDataset(attackner_train_X, attackner_train_y), BATCH_SIZE, shuffle=True
        ),
    ...
    DATASET_LABEL_SIZES, DATASET_VOCAB_SIZES = buildtokenmodeldicts(
        DATASETS, VOCABULARY_SIZES, LABEL_SET_SIZES
    )

    total_batches, batch_sampling_probs = getBatchSamplingProbs(TRAIN_LOADERS)

    # load_dotenv()  # load environment variables if needed
    mlflow.set_experiment("nodeduplication")
    ...
    with mlflow.start_run(run_name="TokenModel_sepEMB_tiedLSTM_train"):
        ...
        for dataset_name in DATASETS:
            dev_X, dev_y, idx2word, idx2label, dev_data = dev_data_dict[
                dataset_name
            ]  # dev_y not currently used - this should change
            # once we've settled on hyperparams, such that we can include it in training.
            with mlflow.start_run(
                run_name=f"TokenModel_sepEMB_tiedLSTM_eval_{dataset_name}", nested=True
            ):
...
if __name__ == "__main__" and DEDUPLICATION == True:
    for intended_eval_dataset in DATASETS:
        cyner_train_data, _ = remove_leakage(
            cyner_train_data, DATASETS_DATAPACK_DEV[intended_eval_dataset]
        )
        attackner_train_data, _ = remove_leakage(
            attackner_train_data, DATASETS_DATAPACK_DEV[intended_eval_dataset]
        )
        aptner_train_data, _ = remove_leakage(
            aptner_train_data, DATASETS_DATAPACK_DEV[intended_eval_dataset]
        )
        dnrti_train_data, _ = remove_leakage(
            dnrti_train_data, DATASETS_DATAPACK_DEV[intended_eval_dataset]
        )
        ...
        TRAIN_LOADERS = {
            "dnrti": DataLoader(
                TensorDataset(dnrti_train_X, dnrti_train_y), BATCH_SIZE, shuffle=True
            ),
            "aptner": DataLoader(
                TensorDataset(aptner_train_X, aptner_train_y), BATCH_SIZE, shuffle=True
            ),
            ...
            "cyner": DataLoader(
                TensorDataset(cyner_train_X, cyner_train_y), BATCH_SIZE, shuffle=True
            ),
        }
        ...
        mlflow.set_experiment(f"train-TokenModel_sepEMB_tiedLSTM-eval-{intended_eval_dataset}")
        with mlflow.start_run(
            run_name=f"train-TokenModel_sepEMB_tiedLSTM-eval-{intended_eval_dataset}"
        ):
            # Log hyperparameters
            hyperparams = {
                "BATCH_SIZE": BATCH_SIZE,
